...
No matter what case works better for you, the first step is always to [create and configure an Edgee project](/proxy/getting-started/project-settings).
...
To get the full benefits of Edgee's performance, you might want to [enable caching on Edgee](/proxy/services/performance/caching).
...
[Learn more about custom domains and SSL certificates](/proxy/getting-started/domains).
...
If the existing CDN is already taking care of caching, you probably want to [keep caching disabled on Edgee](/proxy/services/performance/caching).
...
you could [enable caching on Edgee](/proxy/services/performance/caching).
...
2.  Route traffic from your Edgee endpoint to [to your domain](/proxy/getting-started/routing).
...
4.  Secure the connection by adding [required headers](/proxy/services/security/access-control#required-header) in Edgee. This ensures only authorized requests from Edgee are accepted by your CDN and forwarded to your cloud/website.
...
[Learn more about integrating the Edgee SDK](/proxy/getting-started/sdk).
...
[Learn more about how to collect the track event](/proxy/services/data-collection/track-event).
...
[Learn more about how to collect the user event](/proxy/services/data-collection/user-event).
