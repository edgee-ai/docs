...
<Columns cols={2}>
  ...
  <Card title="Claude Token Compression (Beta)" img="/images/banner-claude-compression.png" href="/features/claude-compression">
...
await edgee.send({ model: 'gpt-5.2', input: 'Hello, world!' });
...
# Access any model with the same key
edgee.send(model='gpt-5.2', input='Hello, world!')
...
// Access any model with the same key
client.send("gpt-5.2", "Hello, world!").await?;
...
// Access any model with the same key
client.Send("gpt-5.2", "Hello, world!")
